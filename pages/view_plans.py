...
def show():
    ...
    if plans:
        ...
        if not days:
            st.error("Unable to parse workout days from this plan file.")
        ...
        if not day_names:
            st.error("No workout days found in this plan.")
            action_button("Back to Dashboard", "dashboard", width="stretch")
            return
            ...
            with st.container():
                ...
                ">
                    ...
                    <div style="margin-top: 0.75rem; display: grid; grid-template-columns: repeat(4, 1fr); gap: 0.5rem;">
                        <div style="border: 1px solid {colors['border_medium']}; border-radius: 10px; padding: 0.5rem; background-color: {colors['surface']};">
